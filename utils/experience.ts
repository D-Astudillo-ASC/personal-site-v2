...
export function calculateYearsByEmploymentType(
  experiences: Experience[],
): EmploymentExperience {
  const professionalExperiences: Experience[] = experiences.filter(
    (exp) => exp.employmentType === "Professional",
  );
  const internshipExperiences: Experience[] = experiences.filter(
    (exp) => exp.employmentType === "Internship",
  );
