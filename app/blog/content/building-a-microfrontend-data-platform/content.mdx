...
After moving a hot API from [**21s to ~250ms**](/blog/cutting-a-data-api-from-21s-to-250ms), the next bottleneck was frontend coordination: **N product UIs, one authenticated session, zero cross-team deploy coupling**. I shipped a platform shell — host, federated remotes, BFF seam — so teams could deploy independently without sharing a monolith repo.

---

## Problem framing

The constraint set was familiar: one login and nav chrome, org-scoped entitlements, and **remote deploy ≠ shell deploy** without paying the iframe tax (shared React, shared tokens).
...
## Auth0 token exchange at the shell boundary

The host exchanges the user’s session for a **downscoped** API token before remotes mount. Remotes never see the full Auth0 access token — they receive a CTE token scoped to the data product’s audience. When a remote’s bundle drifted on `@auth0/auth0-spa-js`, the symptom was silent 401s on BFF calls while the shell still looked logged in — fixed by pinning shared auth packages in the host’s `shared` federation config.

---

## Stub compilation in CI

Each remote builds against **compiled stubs** of sibling remotes in CI, not live `remoteEntry.js` fetches. A missing export fails the build with a type error instead of a white screen in production when team B ships before team A updates the contract.

---

## Closing thought

Latency work optimizes milliseconds on the hot path. Platform work optimizes **contract safety across team boundaries** — federation stubs, Auth0 CTE for token shape, CI for cache/CSP invariants. Hosts pay the tax in CI and runtime; remotes pay it in semver. Both sides must treat shared dependencies as a contract, not a suggestion.

---

## Reader field guide

**When this pattern applies:** Two or more UI teams share one authenticated product surface; releases must decouple; you can invest in CI contracts (stubs, CSP, cache headers) and a platform BFF for token policy. Skip federation if teams always ship together — a monorepo with packages is cheaper.

**Operational checklist**

- Pin **React/react-dom as federation singletons** (`eager: true` on the host) before the first remote loads.
- Check in **compile-time stubs** for every remote `exposes`; fail CI on rename/remove.
- Add a BFF route only when you need **token downscope**, aggregation, or uniform **Cache-Control** — delete pass-through proxies.
- Wire **Auth0 Organizations** + **CTE (RFC 8693)** so each remote gets an audience-scoped token, not the shell’s full access token.
- Lint **`remoteEntry.js` cache policy** and **CSP allowlists** on every remote merge — prod script blocks are expensive.
- Treat **shared auth and router versions** in the host `shared` map as platform semver, not per-team preference.

**Shell vs integration style**

| Situation | Monolith SPA | Iframe mosaic | Module Federation + BFF |
| --- | --- | --- | --- |
| One team, one release train | Best fit | Overkill | Overkill |
| N teams, shared session chrome | Deploy coupling | Weak UX, duplicate auth | Best fit |
| Strict third-party script isolation | OK | Strong isolation | Needs CSP discipline |
| Runtime contract enforcement | Types in-repo | PostMessage hacks | Stubs + CI |
...
| Post | Why |
| --- | --- |
| [Cutting a data API from 21s to ~250ms](/blog/cutting-a-data-api-from-21s-to-250ms) | Prerequisite — why the BFF had to exist on a fast OLTP plane |
| [Bridging AMQPS and JMS for real-time events](/blog/bridging-amqps-and-jms-for-real-time-events) | Async domain events when the UI does not need to wait |

### References (curated)

Federation docs explain the loader graph; Auth0’s token-exchange page is what I re-read when remotes need a narrower audience than the shell.

| Reference | Notes |
| --- | --- |
| [Module Federation concepts](https://module-federation.io/guide/basic/introduction.html) | Shared scope, singletons, and why the host owns React versions. |
| [Auth0: Custom Token Exchange (RFC 8693)](https://auth0.com/docs/authenticate/custom-token-exchange) | Downscoping tokens per remote without handing every team the shell JWT. |
| [Rspack Module Federation plugin](https://module-federation.io/guide/basic/rspack) | Build-time wiring we used alongside compile-time stubs in CI. |
