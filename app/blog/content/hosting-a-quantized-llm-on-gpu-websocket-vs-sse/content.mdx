...
Hindsight: **I should have used SSE or OpenAI-style HTTP streaming** on the pod—the same pattern I already had in [experiment A](/blog/building-a-gemini-ai-backend-with-sse) on `POST /api/chat/message/stream`. WebSocket bought protocol complexity without any bidirectional win.
...
| Time to first token | Only if inference streams; my pod **faked** chunks after batch `generate()` | Real when BFF uses `generateContentStream` ([experiment A](/blog/building-a-gemini-ai-backend-with-sse)) | Real partial tokens from the engine |
...
- [ ] Run `vllm serve` with OpenAI-compatible server ([docs](https://docs.vllm.ai/en/latest/serving/openai_compatible_server.html))
- [ ] BFF calls `stream: true`; mobile only ever hits BFF **SSE** ([experiment A wire format](/blog/building-a-gemini-ai-backend-with-sse))
...
| --- | --- |
| Ship AI features this sprint on a social app | [Experiment A](/blog/building-a-gemini-ai-backend-with-sse) — managed API + SSE |
...
| Reference | Notes |
| --- | --- |
| [vLLM OpenAI-compatible server](https://docs.vllm.ai/en/latest/serving/openai_compatible_server.html) | `stream: true` from the BFF—stop hand-rolling FastAPI WebSockets for text. |
| [vLLM online serving](https://docs.vllm.ai/en/latest/serving/online_serving/) | Deployment patterns (ports, health) on GPU hosts like RunPod. |
| [vLLM Realtime API (Jan 2026)](https://vllm.ai/blog/2026-01-31-streaming-realtime) | WebSocket when the modality is realtime **audio**—not matchmaking blurbs. |
| [MDN: Server-sent events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events) | What the phone should still speak after you add a self-hosted backend. |
